            ...
            <form action="{{ route(app()->getLocale() . '.admin.certificates.update', $certificate) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="language" id="language" class="mt-1 block w-full rounded-md border-[var(--color-primary-300)] shadow-sm focus:border-[var(--color-primary-500)] focus:ring focus:ring-[var(--color-primary-200)]" required>
                        <option value="vi" {{ $certificate->language === 'vi' ? 'selected' : '' }}>{{ __('Tiếng Việt') }}</option>
                        <option value="zh" {{ $certificate->language === 'zh' ? 'selected' : '' }}>{{ __('Tiếng Trung') }}</option>
                    </select>
                    @error('language')
                ...
                <!-- Image -->
                <div>
                    <label for="image" class="block text-sm font-medium text-[var(--color-primary-700)]">{{ __('Hình Ảnh') }}</label>
                    ...
                    <p class="mt-1 text-sm text-[var(--color-primary-600)]">{{ __('Không chọn ảnh mới để giữ ảnh hiện tại') }}</p>
                    @error('image')
                        <p class="mt-1 text-sm text-[var(--color-secondary-500)]">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $certificate->name) }}" class="mt-1 block w-full rounded-md border-[var(--color-primary-300)] shadow-sm focus:border-[var(--color-primary-500)] focus:ring focus:ring-[var(--color-primary-200)]" required>
                    @error('name')
                        <p class="mt-1 text-sm text-[var(--color-secondary-500)]">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="mt-1 block w-full rounded-md border-[var(--color-primary-300)] shadow-sm focus:border-[var(--color-primary-500)] focus:ring focus:ring-[var(--color-primary-200)]">{{ old('description', $certificate->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-[var(--color-secondary-500)]">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" class="rounded border-[var(--color-primary-300)] text-[var(--color-primary-600)] shadow-sm focus:border-[var(--color-primary-500)] focus:ring focus:ring-[var(--color-primary-200)]" value="1" {{ old('is_active', $certificate->is_active) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" value="{{ old('order', $certificate->order) }}" class="mt-1 block w-full rounded-md border-[var(--color-primary-300)] shadow-sm focus:border-[var(--color-primary-500)] focus:ring focus:ring-[var(--color-primary-200)]">
                    @error('order')
                        <p class="mt-1 text-sm text-[var(--color-secondary-500)]">{{ $message }}</p>
                    @enderror
                </div>
